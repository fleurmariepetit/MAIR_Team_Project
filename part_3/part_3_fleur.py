...
while (not sentence_finished) & (tries < max_tries) & (k < len(poss_combs)):
    ...
    while (not sentence_finished) & (type_tries < max_tries/3):
        ...
        backward = re.search(r"^((\w{1,2})|(\(\w{1,2}((F)|(B))\w{1,2}\)))(B)", ccg_type)
        forward = re.search(r"(F)(\w{1,2}$|(\(\w{1,2}((F)|(B))\w{1,2}\)$))", ccg_type)
        last_one = i
        if bool(backward):
            required_backward = re.search(r"^\w{1,2}|(^\(\w{1,2}((F)|(B))w{1,2}\))", ccg_type).group(0)
            ...
            required_forward = re.search(r"\w{1,2}$|\(\w{1,2}((F)|(B))\w{1,2}\)$", ccg_type).group(0)
            ...
            if i != (len(sentence_df)-1):
                ...
                if next_type == required_forward:
                    ...
                    sentence_df.iloc[i, j] = new_type
                    sentence_df.drop(sentence_df.index[i + 1], inplace=True)
        if bool(re.search(r"(B)|(FARD)", sentence_df.iloc[i, j])) & (second_try == False):
            second_try = True
